...
def log_fit_with_uncertainty(x_data, y_data):
    import numpy as np
    ...
    rcParams['font.sans-serif'] = ['Times New Roman']  # SimHei 是常见的中文字体
    rcParams['axes.unicode_minus'] = False  # 解决负号显示问题


    # 绘制结果
    plt.figure(figsize=(8, 6))
    plt.scatter(x_data, y_data, label="数据点", color="blue")  # 原始数据
    plt.plot(x_data, y_fit, label=f"拟合曲线: y = {a:.2f}*ln(x) + {b:.2f}", color="red")  # 拟合曲线
    plt.fill_between(
        x_data,
        y_fit - y_uncertainty,
        y_fit + y_uncertainty,
...
def fit_curve_multiple(x_data, y_data, n_fits=2,line_style='-', line_width=2, scatter_marker='o', scatter_color='blue', line_color='red'):
    ...
    # 绘制拟合曲线
    if n_fits > 2:
        label = f"{n_fits}次多项式拟合: {equation}"
    elif n_fits == 1:
        label = f"线性拟合: {equation}"
    elif n_fits == 2:
        label = f"抛物线拟合: {equation}"
...
def residuals(params, x_data, y_data):
    r, xs = params
    y_fit = tedmonf_equation(x_data, r, xs)
    return np.sum((y_data - y_fit)**2)

# 拟合函数
def tedmonf_fit(x_data, y_data, r_init, xs_init, line_style='-', line_width=2, scatter_marker='o', scatter_color='blue',
                line_color='red'):
    import numpy as np
    import matplotlib.pyplot as plt
    from matplotlib import rcParams
    ...
    # 绘制拟合曲线
    plt.plot(x_data, y_fit, label=f"Tedmonf 拟合曲线: t = {-r:.2f} * x / r - {xs:.2f} * ln(1 - x / xs)",
             color=line_color, linestyle=line_style, linewidth=line_width)

    # 绘制预测范围
    plt.fill_between(x_data, y_fit - y_uncertainty, y_fit + y_uncertainty, color='red', alpha=0.2, label="预测范围")
